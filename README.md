...
Результат компиляции будет помещён в папку `result`.
...
Создайте новые cpp- и hpp-файлы в папке `repo/src` и повторно запустите `2_cmake_vs.bat`. Удаление папки `build_vs` не требуется.
